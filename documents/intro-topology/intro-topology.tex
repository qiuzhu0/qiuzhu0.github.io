...
These notes introduce general topology and some topics of algebraic topology suitable for a strong undergraduate. The general organization is inspired by my notes from MAT 528: Foundations of Topology at Stony Brook University. Some exposure to topological topics from, e.g., an analysis course is helpful.

\section{Metric spaces}

Before delving into the more abstract realm of general topological spaces, let's consider spaces where we at least have a notion of distance, called \emph{metric spaces}.
...
\begin{example}
    $\mathbb{R}$ has the \emph{standard metric} given by $d(x, y) = |x - y|$, and more generally $\mathbb{R}^n$ has the Euclidean metric given by
    \[
        d(x, y) = \sqrt{\sum_{i = 1}^n (x_i - y_i)^2}.
    \]
\end{example}
\begin{example}
    Any set $X$ admits the \emph{discrete metric}
    \[
    d(x, y) =
    \begin{cases}
        0 & \text{if } x = y, \\
...
\begin{definition}[balls]
    ...
    \]
    Replacing $<$ by $\leq$ yields \emph{closed balls}.
\end{definition}

Open balls in the most familiar setting $\mathbb{R}^n$ with the standard metric are open intervals for $n = 1$, the interior region given by a circle for $n = 2$, and the interior region given by a sphere for $n = 3$.
...
\begin{example}
    ...
    \[
    \tau_{\mathbb{R}} = \left\{\bigcup_{\alpha \in A} (a_\alpha, b_\alpha) \mid (a_\alpha, b_\alpha) \subseteq \mathbb{R}\right\}.
    \]
...
\begin{definition}[topological continuity]
    A function $f : X \to Y$ between topological spaces is \emph{continuous} if all preimages of open sets are open.
\end{definition}

This is a natural generalization of the result given by \cref{contpreimage}. Of course, continuity at a point can be described by preimages of \emph{neighborhoods} of the point being open, just as in the specific metric space case.

\begin{definition}[the category of topological spaces]
    The category of topological spaces $\mathbf{Top}$ has topological spaces as objects and continuous functions as arrows.
\end{definition}
...
\begin{theorem}
    ...
    \begin{enumerate}[(a)]
        ...
        \item functions such that all preimages of closed sets are closed.
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    \begin{enumerate}[(a)]
        \item
            Preimages of constant functions are either going to be the empty set or else the entire domain, both which are open.
        \item
            $f^{-1}(U) = U$ for identity functions, so pulling back an open $U$ is still an open set $U$.
        \item
            Let $X \xrightarrow{f} Y \xrightarrow{g} Z$ and let $U \subseteq Z$ be open. Then $g^{-1}(U)$ is open by continuity of $g$, so $f^{-1}(g^{-1}(U))$ is open by continuity of $f$, therefore $g \circ f$ is continuous.
        \item
            Suppose $f : X \to Y$ is such that preimages of closed sets are closed. Let $U \subseteq Y$ be open. Then $f^{-1}(Y \smallsetminus U)$ is closed, so $Y \smallsetminus f^{-1}(Y \smallsetminus U) = f^{-1}(Y \smallsetminus (Y \smallsetminus U)) = f^{-1}(U)$ is open.
    \end{enumerate}
\end{proof}

\begin{definition}[homeomorphisms]
    A bijection $f : X \to Y$ is a \emph{homeomorphism} if it is continuous and its inverse is continuous. If a homeomorphism $X \to Y$ exists, $X$ and $Y$ are \emph{homeomorphic}, denoted by $X \cong Y$.
\end{definition}

\begin{remark}
    Being homeomorphic is an equivalence relation.
\end{remark}

\begin{definition}[topological embeddings]
    A function $f : X \to Y$ is a \emph{topological embedding} if the subfunction $X \to f(X)$ given by $x \mapsto f(x)$ is a homeomorphism.

    Two embeddings $f_1, f_2 : X \to Y$ are \emph{equivalent} if this diagram in $\mathbf{Top}$ commutes for some $h_X, h_Y$:
    \[\begin{tikzcd}
        X & Y \\
        X & Y
